                ...
                .forEach(post => {
                  ...
                  suggestions.push(
                    makeSuggestion(user, '@' + user.username() + '#' + post.id(), [
                      app.translator.trans('starinc-nex_comment-tree.forum.composer.reply_to_post_text', {number: post.number()}), ' — ',
  ...
  extend(TextEditor.prototype, 'toolbarItems', function(items) {
    items.add('mention', (
      <TextEditorButton onclick={() => this.insertAtCursor('@')} icon="fas fa-at">
        {app.translator.trans('starinc-nex_comment-tree.forum.composer.mention_tooltip')}
      </TextEditorButton>
    ));
  });
}
