  ...
  extend(CommentPost.prototype, 'footerItems', function(items) {
    ...
    if (replies && replies.length) {
        ...
        .map(reply => {
          ...
          return (
            <a href={app.route.post(reply)}
              ...
              {app.session.user === user ? app.translator.trans('starinc-nex_comment-tree.forum.post.you_text') : username(user)}
            </a>
          );
        });
      ...
      if (overLimit) {
        ...
        names.push(
          app.translator.transChoice('starinc-nex_comment-tree.forum.post.others_text', count, {count})
        );
      }
      ...
      items.add('replies',
        <div className="Post-mentionedBy" config={config}>
          <span className="Post-mentionedBy-summary">
            {icon('fas fa-reply')}
            {app.translator.transChoice('starinc-nex_comment-tree.forum.post.mentioned_by' + (repliers[0].user() === app.session.user ? '_self' : '') + '_text', names.length, {
              count: names.length,
